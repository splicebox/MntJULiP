...
#### DSR analysis: *diff_spliced_introns.txt* & *diff_spliced_groups.txt*
...
It lists all introns individually along with their genomic location, status of the test (*OK* or *LO_DATA*), and the raw read counts for that intron in all samples, grouped by condition. This information can be used, for instance, to further filter introns with low support across all or subsets of the samples, or from genes with low expression levels.
